...
def Login(request):
    if "username" in request.session:
        ...
        if request.method=='POST':
            ...
            return HttpResponse(LoginUser(request,Message=True))
    
def LoginUser(request,Message=None):
    if "username" in request.session:
        return HttpResponseRedirect('/UserLogin/')
    else:
        if Message is None:
            c={'UserData':False}
            return render_to_response(template_name='Login.html', context=None, context_instance=RequestContext(request))
        else:
            c={'UserData':True}
            return render_to_response(template_name='Login.html', context=c, context_instance=RequestContext(request))       
